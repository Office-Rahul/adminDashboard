...
const Distributor = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("http://localhost:8000/getDistributor");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingUser) {
        res = await axios.put(
          `http://localhost:8000/updateDistributor/${editingUser._id}`,
          {
            userName,
            password,
            commission,
        ...
        res = await axios.post("http://localhost:8000/createDistributor", {
          userName,
          password,
          commission,
        });
  ...
  const handleDelete = async (userId) => {
    try {
      const response = await axios.delete(
        `http://localhost:8000/deleteDistributor/${userId}`
      );
      if (response.data.status === true) {
  ...
  const toggleUserStatus = async (userId, newStatus) => {
    try {
      await axios.put(`http://localhost:8000/updateDistributor/${userId}`, {
        status: newStatus,
      });
